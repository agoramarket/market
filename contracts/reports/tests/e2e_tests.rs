...
use reports::{
    Error as ReportError, EstadisticasCategoria, ProductoVendido, Reportes, ReportesRef,
...
async fn e2e_generacion_reportes(mut client: Client) -> E2EResult<()> {
    ...
    let reports_call = reports_contract.call_builder::<Reportes>();
    ...
    client
        .call(&ink_e2e::charlie(), &reg_charlie)
        .submit()
        .await
        .expect("reg charlie failed");
    ...
    client
        .call(&ink_e2e::alice(), &env_bob)
        .submit()
        .await
        .expect("env bob failed");

    let rec_bob = market_call.marcar_recibido(oid_bob);
    client
        .call(&ink_e2e::bob(), &rec_bob)
        .submit()
        .await
        .expect("rec bob failed");

    let calif_bob = market_call.calificar_vendedor(oid_bob, 5);
    client
        .call(&ink_e2e::bob(), &calif_bob)
        .submit()
        .await
    ...
    client
        .call(&ink_e2e::alice(), &env_charlie)
        .submit()
        .await
        .expect("env charlie failed");

    let rec_charlie = market_call.marcar_recibido(oid_charlie);
    client
        .call(&ink_e2e::charlie(), &rec_charlie)
        .submit()
        .await
        .expect("rec charlie failed");

    let calif_charlie = market_call.calificar_vendedor(oid_charlie, 3);
    client
        .call(&ink_e2e::charlie(), &calif_charlie)
        .submit()
        .await
        .expect("calif charlie failed");
    ...
    let result = client
        .call(&ink_e2e::alice(), &resumen_msg)
        .submit()
        .await
        .expect("resumen failed");
    ...
    let result = client
        .call(&ink_e2e::alice(), &top_vend_msg)
        .submit()
        .await
        .expect("top_vend failed");
    ...
    let result = client
        .call(&ink_e2e::alice(), &mas_vendidos_msg)
        .submit()
        .await
        .expect("mas_vendidos failed");
    ...
    let result = client
        .call(&ink_e2e::alice(), &stats_cat_msg)
        .submit()
        .await
        .expect("stats_cat failed");
