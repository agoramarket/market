...
//!
//! ```rust,ignore
//! // Este ejemplo muestra el flujo típico de uso del marketplace
//! // En la práctica, estas operaciones se ejecutarían desde diferentes transacciones
//!
//! // Crear una nueva instancia del marketplace
//! let mut marketplace = Marketplace::new();
//!
//! // Registro de vendedor (Alice)
//! marketplace.registrar(Rol::Vendedor).unwrap();
//!
//! // Publicar un producto
//! let producto_id = marketplace.publicar("Laptop Gaming".to_string(), 1500, 5).unwrap();
//!
//! // Cambio de usuario a comprador (Bob) y realizar una compra
...
mod marketplace {
    ...
    impl Marketplace {
        ...
        /// # Arguments
        ///
        /// * `usuario` - La dirección del usuario cuyo rol se quiere consultar
        ///
        /// # Returns
        ///
        ...
        ///
        /// ```rust,ignore
        /// match marketplace.obtener_rol(user_account) {
        ...
        pub fn publicar(
            ...
            self._publicar(nombre, precio, stock)
        }
        ...
        ///
        /// ```rust,ignore
        /// if let Some((vendedor, nombre, precio, stock)) = marketplace.obtener_producto(1) {
        ///     println!("Producto: {} - Precio: {} - Stock: {}", nombre, precio, stock);
        /// }
        /// ```
        #[ink(message)]
        pub fn obtener_producto(&self, id: u32) -> Option<(AccountId, String, Balance, u32)> {
            self._obtener_producto(id)
        }

        /* ───────────────────────────────────────────────────────
         *  MENSAJES PÚBLICOS – ÓRDENES
         * ───────────────────────────────────────────────────────*/
        ...
        ///
        /// ```rust,ignore
        /// if let Some((comprador, vendedor, producto_id, cantidad, estado)) = marketplace.obtener_orden(1) {
        ///     match estado {
        ...
        /// ```
        #[ink(message)]
        pub fn obtener_orden(&self, id: u32) -> Option<(AccountId, AccountId, u32, u32, Estado)> {
            self._obtener_orden(id)
        }

        /* ───────────────────────────────────────────────────────
         *  FUNCIONES PRIVADAS – ROLES
         * ───────────────────────────────────────────────────────*/

        /// Implementación interna de la lógica de registro de usuarios.
        ///
        /// Esta función privada contiene la lógica de negocio para registrar usuarios,
        /// incluyendo todas las validaciones y el almacenamiento del rol.
        ///
        /// # Arguments
        ///
        /// * `rol` - El rol que el usuario desea tener en el marketplace
        ...
        ///
        /// `Ok(())` si el registro es exitoso, `Err(Error)` si hay algún problema.
        fn _registrar(&mut self, rol: Rol) -> Result<(), Error> {
        ...
        /// Obtiene el rol de un usuario basado en su AccountId (versión interna).
        ///
        ...
        fn _rol_interno(&self, who: AccountId) -> Result<Rol, Error> {
        ...
        /// Implementación interna de la consulta de roles de usuario.
        ///
        /// Esta función privada contiene la lógica para obtener el rol de un usuario
        /// desde el almacenamiento del contrato, incluyendo la conversión del valor numérico
        /// a la enumeración de rol correspondiente.
        ///
        /// # Arguments
        ///
        ...
        /// `Some(Rol)` con el rol del usuario si está registrado,
        /// `None` si el usuario no está registrado.
        fn _obtener_rol(&self, user: AccountId) -> Option<Rol> {
            self.roles.get(user).and_then(|v| match v {
                0 => Some(Rol::Comprador),
                1 => Some(Rol::Vendedor),
                2 => Some(Rol::Ambos),
                _ => None,
            })
        ...
        ///
        /// ```rust,ignore
        /// marketplace.ensure(precio > 0, Error::ParamInvalido)?;
        /// ```
        ...
        fn assert_comprador(&self, who: AccountId) -> Result<(), Error> {
            self.ensure(self._rol_interno(who)?.es_comprador(), Error::SinPermiso)
        }
        ...
        fn assert_vendedor(&self, who: AccountId) -> Result<(), Error> {
            self.ensure(self._rol_interno(who)?.es_vendedor(), Error::SinPermiso)
        }

        /* ───────────────────────────────────────────────────────
         *  FUNCIONES PRIVADAS – PRODUCTOS
         * ───────────────────────────────────────────────────────*/

        /// Implementación interna de la lógica de publicación de productos.
        ///
        /// Esta función privada contiene la lógica de negocio para publicar productos,
        /// incluyendo todas las validaciones, verificación de permisos y asignación de IDs.
        ///
        /// # Arguments
        ///
        /// * `nombre` - El nombre del producto (máximo 64 caracteres)
        ...
        ///
        /// `Ok(u32)` con el ID del producto creado, `Err(Error)` si hay algún problema.
        fn _publicar(&mut self, nombre: String, precio: Balance, stock: u32) -> Result<u32, Error> {
            ...
            self.productos
                .insert(pid, &(vendedor, nombre, precio, stock));
            Ok(pid)
        }

        /// Implementación interna de la consulta de productos.
        ///
        /// Esta función privada contiene la lógica para obtener información de productos
        /// desde el almacenamiento del contrato.
        ///
        /// # Arguments
        ///
        /// * `id` - El ID del producto a consultar
        ...
        /// `Some((AccountId, String, Balance, u32))` con los datos del producto si existe,
        /// `None` si el producto no existe.
        fn _obtener_producto(&self, id: u32) -> Option<(AccountId, String, Balance, u32)> {
            self.productos.get(id)
        }

        /* ───────────────────────────────────────────────────────
         *  FUNCIONES PRIVADAS – ÓRDENES
         * ───────────────────────────────────────────────────────*/

        /// Implementación interna de la lógica de compra de productos.
        ///
        /// Esta función privada contiene la lógica de negocio para realizar compras,
        /// incluyendo validaciones de permisos, stock, y creación de órdenes.
        ///
        /// # Arguments
        ///
        /// * `id_prod` - El ID del producto a comprar
        ...
        ///
        /// `Ok(u32)` con el ID de la orden creada, `Err(Error)` si hay algún problema.
        fn _comprar(&mut self, id_prod: u32, cant: u32) -> Result<u32, Error> {
            ...
            self.ordenes
                .insert(oid, &(comprador, p.0, id_prod, cant, 0));
            Ok(oid)
        }

        /// Implementación interna de la lógica para marcar órdenes como enviadas.
        ///
        /// Esta función privada contiene la lógica de negocio para cambiar el estado
        /// de una orden a "Enviado", incluyendo todas las validaciones de permisos y estado.
        ///
        /// # Arguments
        ///
        /// * `oid` - El ID de la orden a marcar como enviada
        ...
        ///
        /// `Ok(())` si la operación es exitosa, `Err(Error)` si hay algún problema.
        fn _marcar_enviado(&mut self, oid: u32) -> Result<(), Error> {
            let caller = self.env().caller();
        ...
        /// Implementación interna de la lógica para marcar órdenes como recibidas.
        ///
        /// Esta función privada contiene la lógica de negocio para cambiar el estado
        /// de una orden a "Recibido", incluyendo todas las validaciones de permisos y estado.
        ///
        /// # Arguments
        ///
        /// * `oid` - El ID de la orden a marcar como recibida
        ...
        ///
        /// `Ok(())` si la operación es exitosa, `Err(Error)` si hay algún problema.
        fn _marcar_recibido(&mut self, oid: u32) -> Result<(), Error> {
            let caller = self.env().caller();
            let mut o = self.ordenes.get(oid).ok_or(Error::OrdenInexistente)?;
        ...
        /// Implementación interna de la consulta de órdenes.
        ///
        /// Esta función privada contiene la lógica para obtener información de órdenes
        /// desde el almacenamiento del contrato, incluyendo la conversión del estado numérico
        /// a la enumeración correspondiente.
        ///
        /// # Arguments
        ///
        /// * `id` - El ID de la orden a consultar
        ...
        /// `Some((AccountId, AccountId, u32, u32, Estado))` con los datos de la orden si existe,
        /// `None` si la orden no existe.
        fn _obtener_orden(&self, id: u32) -> Option<(AccountId, AccountId, u32, u32, Estado)> {
    ...
    mod tests {
        ...
        fn test_flujo_completo_marketplace() {
            ...
            assert_eq!(marketplace.obtener_rol(accounts.alice), Some(Rol::Vendedor));

            // Test error: ya registrado
            assert_eq!(
                marketplace.registrar(Rol::Comprador),
                Err(Error::YaRegistrado)
            );

            // Test 2: Registro de comprador
            set_caller(accounts.bob);
            assert_eq!(marketplace.registrar(Rol::Comprador), Ok(()));
            assert_eq!(marketplace.obtener_rol(accounts.bob), Some(Rol::Comprador));
            ...
            set_caller(accounts.bob);
            assert_eq!(
                marketplace.publicar("Item".to_string(), 100, 1),
        ...
        fn test_errores_y_validaciones() {
            ...
            assert_eq!(
                marketplace.publicar("".to_string(), 0, 1),
                Err(Error::ParamInvalido)
            );
            assert_eq!(
                marketplace.publicar("Item".to_string(), 100, 0),
                Err(Error::ParamInvalido)
            );
            assert_eq!(
                marketplace.publicar("a".repeat(65), 100, 1),
                Err(Error::ParamInvalido)
            );

            // Publicar producto válido
            marketplace
                .publicar("Producto".to_string(), 100, 2)
                .unwrap();
            ...
            // Errores en órdenes
            assert_eq!(
                marketplace.marcar_enviado(999),
                Err(Error::OrdenInexistente)
            );
            assert_eq!(
                marketplace.marcar_recibido(999),
                Err(Error::OrdenInexistente)
            );
        ...
        fn test_roles_y_permisos() {
            ...
            assert_eq!(marketplace.obtener_rol(accounts.alice), Some(Rol::Ambos));

            // Usuario con rol "Ambos" puede publicar
            let producto_id = marketplace
                .publicar("Producto".to_string(), 500, 3)
                .unwrap();
            ...
            assert!(marketplace.obtener_producto(999).is_none());
            assert!(marketplace.obtener_orden(999).is_none());
            assert!(marketplace.obtener_rol(accounts.bob).is_none());
        }
    }
}
